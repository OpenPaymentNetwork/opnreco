...
@view_config(name="", context=FileCollection, permission=perms.use_app, renderer="json")
def list_files(context, request, archived=False):
    ...
    if selected_period_id:
        ...
        row = (
            ...
            .filter(Period.owner_id == owner_id, Period.id == int(selected_period_id))
            .first()
        )
        if row:
    ...
    file_rows = (
        dbsession.query(File, open_subq, closed_subq)
        .filter(File.owner_id == owner_id, file_filter)
        .order_by(File.title)
        .all()
    )
    ...
    if selected_period_id:
        period_filter = or_(period_filter, subq.c.id == int(selected_period_id))
    period_rows = dbsession.query(period_alias).filter(period_filter).all()
...
@view_config(
    name="archived", context=FileCollection, permission=perms.use_app, renderer="json"
)
def list_archived_files(context, request):
    return list_files(context, request, archived=True)


@view_config(name="", context=FileResource, permission=perms.view_file, renderer="json")
def file_state(context, request):
    return serialize_file(context.file)


class FileSaveSchema(colander.Schema):
    title = colander.SchemaNode(colander.String(), validator=colander.Length(max=50))
    auto_enable_loops = colander.SchemaNode(colander.Boolean(), missing=None)
    reinterpret = colander.SchemaNode(colander.Boolean(), missing=False)


@view_config(
    name="save", context=FileResource, permission=perms.edit_file, renderer="json"
)
def file_save(context, request):
    ...
    if appstruct["reinterpret"]:
        ...
        dbsession = request.dbsession
        query = dbsession.query(FileSync).filter(FileSync.file_id == file.id)
...
@view_config(
    name="archive", context=FileResource, permission=perms.edit_file, renderer="json"
)
...
class FileAddSchema(colander.Schema):
    ...
    title = colander.SchemaNode(colander.String(), validator=colander.Length(max=100))
    peer_id = colander.SchemaNode(
        colander.String(), validator=colander.Length(max=50), missing=None
    )
    auto_enable_loops = colander.SchemaNode(colander.Boolean(), missing=False)
...
@view_config(
    name="add", context=FileCollection, permission=perms.use_app, renderer="json"
)
...
def page_loop_configs(request, file, final):
    ...
    query = (
        ...
        .outerjoin(
            Loop,
            and_(Loop.owner_id == owner_id, Loop.loop_id == FileLoopConfig.owner_id),
        )
        .outerjoin(
            Peer,
            and_(Peer.owner_id == owner_id, Peer.peer_id == FileLoopConfig.issuer_id),
        )
        .filter(
    ...
    peer_map = get_peer_map(request=request, need_peer_ids=need_peer_ids, final=final)
    loop_map = get_loop_map(request=request, need_loop_ids=need_loop_ids, final=final)
...
@view_config(
    name="loops", context=FileResource, permission=perms.view_file, renderer="json"
)
...
def configure_loops_api(context, request):
    ...
    if movement_filters:
        ...
        transfer_record_ids = (
            ...
            .filter(Movement.owner_id == owner_id, or_(*movement_filters))
            .distinct()
        )

        query = dbsession.query(FileSync).filter(
            FileSync.file_id == file.id,
            FileSync.transfer_record_id.in_(transfer_record_ids.subquery("subq")),
        )
        query.delete(synchronize_session=False)
