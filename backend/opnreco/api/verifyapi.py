...
@view_config(name="verify", context=API, permission=perms.use_app, renderer="json")
class VerifyAPI(SyncBase):
    ...
    def verify_sync(self):
        ...
        if transfers_download["more"]:
            ...
            progress_percent = min(
                99,
                int(100.0 * ivr.sync_done / ivr.sync_total if ivr.sync_total else 0.0),
            )
        else:
            self.verify_final()
            progress_percent = 100
    ...
    def log_download(self, transfers_download):
        ...
        verified = {item["id"]: None for item in transfers_download["results"]}
        ...
        ivr = self.ivr
        if self.created_ivr:
            ivr.first_sync_ts = to_datetime(transfers_download["first_sync_ts"])
        ...
        ivr.last_sync_ts = to_datetime(transfers_download["last_sync_ts"])
        ivr.last_sync_transfer_id = transfers_download["results"][-1]["id"]
    ...
    def verify_final(self):
        ...
        missing_transfer_ids = stored_transfer_ids.difference(verified_transfer_ids)
    ...
    def verify_internal(self):
        ...
        periods = (
            ...
            .order_by(Period.file_id, Period.start_date)
            .all()
        )
        ...
        for period in periods:
            ...
            if (
                period.start_circ != prev_period.end_circ
                or period.start_surplus != prev_period.end_surplus
            ):
                msg = (
                    ...
                    % (
                        ...
                        period.start_circ,
                        period.start_surplus,
                        prev_period.end_circ,
                        prev_period.end_surplus,
                    )
                )
                raise VerificationFailure(msg, transfer_id=None)
...
@view_config(
    name="verify-details", context=API, permission=perms.use_app, renderer="json"
)
