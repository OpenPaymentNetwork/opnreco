...
from opnreco.param import amount_re, parse_amount
from opnreco.reassign import reassign_statement_period
from opnreco.viewcommon import bad_request, configure_dblog, list_assignable_periods
...
def get_delete_conflicts(dbsession, statement):
    ...
    entries_in_closed_period = (
        ...
        .filter(AccountEntry.statement_id == statement.id, Period.closed)
        .scalar()
    )
...
def statement_api(context, request):
    ...
    if statement is None:
        raise HTTPBadRequest(
            json_body={
                ...
                "error_description": (
                    "Statement %s not found in this period." % statement_id_input
                ),
            }
        )
    ...
    delete_conflicts = get_delete_conflicts(dbsession=dbsession, statement=statement)
    ...
    if statement is None:
        raise HTTPBadRequest(
            json_body={
                "error": "statement_not_found",
                "error_description": (
                    "Statement %s not found in this period." % statement_id
                ),
            }
        )
    ...
    if content is None:
        raise HTTPBadRequest(
            json_body={
                ...
                "error_description": ("Statement %s was not uploaded." % statement_id),
            }
        )

    headers = {
        "Content-Disposition": 'attachment; filename="%s"' % (statement.filename),
...
class StatementSaveSchema(colander.Schema):
    ...
    source = colander.SchemaNode(colander.String(), validator=colander.Length(max=100))
...
def statement_save(context, request):
    ...
    if statement is None:
        raise HTTPBadRequest(
            json_body={
                "error": "statement_not_found",
                "error_description": ("Statement %s not found." % appstruct["id"]),
            }
        )
    ...
    if new_period is not None:
        ...
        configure_dblog(request=request, event_type="reassign_statement_period")
...
class AccountEntryEditSchema(colander.Schema):
    ...
    sheet = colander.SchemaNode(
        colander.String(), missing="", validator=colander.Length(max=50)
    )
    row = colander.SchemaNode(colander.Integer(), missing=None)
    description = colander.SchemaNode(
        colander.String(), missing="", validator=colander.Length(max=1000)
    )
        ...
        if entry is None:
            raise HTTPBadRequest(
                json_body={
                    ...
                    "error_description": ("The specified account entry is not found."),
                }
            )
...
class StatementUploadSchema(colander.Schema):
    ...
    name = colander.SchemaNode(
        colander.String(), validator=colander.Length(min=0, max=1000), missing=""
    )
    size = colander.SchemaNode(colander.Integer(), missing=None)
    type = colander.SchemaNode(
        colander.String(), validator=colander.Length(max=1000), missing=""
    )
...
class StatementUploadAPI:
    ...
    old_excel_types = ("application/vnd.ms-excel",)
    old_excel_extensions = (".xls",)
    open_excel_types = (
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
    open_excel_extensions = (".xlsx",)
    ...
    def __call__(self):
        ...
        if content_type in self.old_excel_types or ext in self.old_excel_extensions:
            self.handle_old_excel()
        elif content_type in self.open_excel_types or ext in self.open_excel_extensions:
            self.handle_open_excel()
        ...
        if self.statement is None:
            raise HTTPBadRequest(
                json_body={
                    ...
                    "error_description": (
                        "File type not supported: %s (%s)" % (ext, appstruct["type"])
                    ),
                }
            )
    ...
    def add_statement(self, source):
        ...
        configure_dblog(request=self.request, account_entry_event_type="upload")
    ...
    def parse_excel_cell(self, book, cell, column_name):
        ...
        if column_name in ("date", "entry date", "entry_date"):
            if hasattr(book, "datemode") and isinstance(cell.value, (int, float)):
        ...
        if column_name in ("amount", "delta"):
            parsed = parse_amount(str(cell.value).strip(), currency=self.currency)
            return "delta", parsed
    ...
    def handle_open_excel(self):
        """Handle a .xlsx (Excel open format) upload"""
        period = self.context.period
        file = period.file
        dbsession = self.request.dbsession
        owner = self.request.owner
        ...
        content: bytes = self.content  # type: ignore
        try:
            ...
            raise HTTPBadRequest(
                json_body={
                    "error": "parse_error",
                    "error_description": (
                        f"Unable to parse uploaded file: {type(e)}: {e}"
                    ),
                }
            )
        ...
        for sheetx, sheet in enumerate(book):
            # Look for a heading row.
            # The heading must contain at least "date" and "amount".
            column_names = None
            heading_rowx = -1
            for rowx, row in enumerate(sheet.rows):
                texts = [str(cell.value).strip().lower() for cell in row]
                if "date" in texts and "amount" in texts:
                    # Found the heading.
                    column_names = tuple(texts)
            ...
            sheet_name = book.sheetnames[sheetx].strip() or str(sheetx + 1)
            if sheet_name.lower().startswith("sheet"):
                # Remove the redundant word.
                sheet_name = sheet_name[5:].strip()
