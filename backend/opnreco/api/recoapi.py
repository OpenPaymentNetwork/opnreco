...
from colander import Integer, Invalid, Length, OneOf, Schema, SchemaNode, Sequence
...
@view_config(
    name="reco", context=PeriodResource, permission=perms.view_period, renderer="json"
)
def reco_api(context, request, final=False):
    ...
    try:
        ...
        raise HTTPBadRequest(
            json_body={"error": "bad reco_id, movement_id, or account_entry_id"}
        )
        ...
        if movement_id is not None:
            movement_rows = (
                start_movement_query(
                    dbsession=dbsession, owner_id=owner_id, file_id=period.file_id
                )
                .filter(
    ...
    if reco_id is not None:
        ...
        reco_period_row = (
            ...
            .filter(Reco.owner_id == owner_id, Reco.id == reco_id)
            .first()
        )
    ...
    loops = get_loop_map(request=request, need_loop_ids=need_loop_ids, final=final)
...
def reco_search_movement(context, request, final=False):
    ...
    if match is not None:
        ...
        filters.append(FileMovement.currency.like(func.concat("%", currency, "%")))
        ...
        try:
            ...
            if parsed is not None:
                ...
                if colon_count >= 2:
                    ...
                    filters.append(FileMovement.ts < ts + datetime.timedelta(seconds=1))
                elif colon_count >= 1:
                    ...
                    filters.append(FileMovement.ts < ts + datetime.timedelta(days=1))
...
def reco_search_account_entries(context, request, final=False):
    ...
    if delta_parsed is not None:
        ...
        if "." in delta_parsed.amount_input:
            # Exact amount.
            filters.append(
                and_(func.abs(AccountEntry.delta) == delta_abs, *sign_filters)
            )
        else:
            # The search omitted the subunit value.
            filters.append(
    ...
    if match is not None:
        currency = match.group(0).upper()
        filters.append(AccountEntry.currency.like(func.concat("%", currency, "%")))
    ...
    if description_input:
        filters.append(
            AccountEntry.description.ilike(func.concat("%", description_input, "%"))
        )
...
class RecoSchema(Schema):
    ...
    comment = SchemaNode(ColanderString(), missing="", validator=Length(max=10000))
    movements = SchemaNode(
        Sequence(), MovementSchema(), missing=(), validator=Length(max=10000)
    )
    account_entries = SchemaNode(
        Sequence(), AccountEntrySchema(), missing=(), validator=Length(max=10000)
    )
...
class RecoSave:
    ...
    def __call__(self):
        ...
        self.check_type_and_balance(
            new_movements=new_movements, new_account_entries=new_account_entries
        )
        ...
        if self.reco_id is not None:
            ...
            self.remove_old_account_entries(new_account_entries=new_account_entries)
        else:
            if (
                not new_movements
                and not new_account_entries
                and not params["reco"]["comment"]
            ):
            ...
            if wallet_sum + vault_sum + entries_sum != zero:
                raise HTTPBadRequest(
                    json_body={
                        ...
                        "account changes: %s)" % (wallet_sum, vault_sum, entries_sum),
                    }
                )
    ...
    def remove_old_movements(self, new_movements):
        ...
        if new_movements:
            filters.append(
                ~FileMovement.movement_id.in_(m.movement_id for m in new_movements)
            )
    ...
    def remove_old_account_entries(self, new_account_entries):
        ...
        new_account_entry_ids = [e.id for e in new_account_entries if e.id is not None]
        filters = []
