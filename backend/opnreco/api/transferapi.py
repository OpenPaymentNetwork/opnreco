...
from opnreco.models.db import FileMovement, Movement, TransferRecord
from opnreco.models.site import PeriodResource
from opnreco.viewcommon import get_loop_map, get_peer_map
...
def transfer_record_api(context, request, final=False):
        ...
        raise HTTPBadRequest(
            json_body={
                ...
                "error_description": (
                    ...
                    "database for %s." % (transfer_id_input, owner.title)
                ),
            }
        )
    ...
    movement_rows = (
        ...
        .filter(Movement.owner_id == owner_id, Movement.transfer_record_id == record.id)
        .order_by(
            Movement.number,
    ...
    peers = get_peer_map(request=request, need_peer_ids=need_peer_ids, final=final)
    ...
    zero = Decimal()
    delta_totals = collections.defaultdict(
        lambda: {
            "vault": zero,
            "wallet": zero,
        }
